%% Write_xlsx_Radiomics2D()
% LISCOMP Lab 2021- 2022, https://liscomp.dima.unige.it
% -------------------------------------------------------------------------
% DESCRIPTION: 
% This function writes the xlsx file containing the 2D radiomics features
% for each slice associated to a ROI.
% -------------------------------------------------------------------------
%%%% called by: radiomics_general2D()

function Write_xlsx_Radiomics2D(number_of_slices_ROIonly_field_name, globalTextures_field_name,...
...
for val = 1 : Nval
    if ROI{val}.Enable
        ...
        aux1 = getfield(radiomics2D{1,val},globalTextures_field_name);
        aux2 = getfield(radiomics2D{1,val},nonTexture_field_name);
        aux3 = getfield(radiomics2D{1,val},matrix_based_textures_field_name);
