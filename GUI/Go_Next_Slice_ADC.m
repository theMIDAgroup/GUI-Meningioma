...
% gui_ADC and sets the following slice as the new current slice.
% -------------------------------------------------------------------------
%%%% called by: GUI_Check_ADC()
%%%% call: Show_ROI_ADC()

function Go_Next_Slice_ADC()
global ROI;
global gui_ADC;
...
if gui_ADC.it2show ~= gui_ADC.SlicesList{gui_ADC.val}(gui_ADC.NumSlices{gui_ADC.val})
    ...
    if gui_ADC.it2show == gui_ADC.SlicesList{gui_ADC.val}(gui_ADC.NumSlices{gui_ADC.val}) &&...
            gui_ADC.val == max(gui_ADC.idx_ROI_enabled)
        set(gui_ADC.Next_slice,'enable','off')
        set(gui_ADC.Done,'enable','on')
    elseif gui_ADC.it2show == gui_ADC.SlicesList{gui_ADC.val}(gui_ADC.NumSlices{gui_ADC.val}) &&...
            gui_ADC.val ~= max(gui_ADC.idx_ROI_enabled)
...
if gui_ADC.val == min(gui_ADC.idx_ROI_enabled) && gui_ADC.it2show == 1
    set(gui_ADC.Previous_slice,'enable','off')
else
    set(gui_ADC.Previous_slice,'enable','on')
end

if gui_ADC.val == max(gui_ADC.idx_ROI_enabled) &&...
        gui_ADC.it2show == gui_ADC.SlicesList{gui_ADC.val}(gui_ADC.NumSlices{gui_ADC.val})
    set(gui_ADC.Next_slice,'enable','off')
    set(gui_ADC.Done,'enable','on')
else
