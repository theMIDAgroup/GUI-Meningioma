...
% the following slice, according to the choice made in the following 
% slice, example: 
% 
...
% CASE 1: the next choice is = 1
if ROI{gui_T1.val}.SliceChoice(gui_T1.SlicesList{gui_T1.val}(gui_T1.k + 1))==1

    % If I replace with choice 1 from next slice, then choice for this slice becomes 1
...
% CASE 2: the next choice is = 2
elseif ROI{gui_T1.val}.SliceChoice(gui_T1.SlicesList{gui_T1.val}(gui_T1.k + 1))==2
    
    if ROI{gui_T1.val}.AlternativeSegmentationList(gui_T1.k) ~= 0 % if the second panel exists
        
        % If I replace with choice 2 from next slice, then choice for this slice becomes 2
...
% Note that ROI{gui_T1.val}.first_next(gui_T1.it2show) = 1;
Show_ROI(gui_T1.val,gui_T1.it2show);  
