...
%inizio aggiunta isa!!! controllo se per caso la mia mask ha una sola fetta
%con questo elseif
 ...
 elseif numel(size(mask))==2
     mask = imresize(mask,[round(double(size(mask,1))),round(double(size(mask,2)))],'nearest');
 end
%%% fine aggiunta
